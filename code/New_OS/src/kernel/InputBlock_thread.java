...
public class InputBlock_thread {
    ...
    public static Condition inputBlockCondition;
    public Process pcb;
    static CPU cpu = ProcessScheduling_thread.cpu;
    public InputBlock_thread(ReentrantLock lock, Condition inputBlockCondition){
        this.lock = lock;
        this.inputBlockCondition = inputBlockCondition;
    }
    //输入中断阻塞记录表
    public static List<Integer> block1ProcessIds = new ArrayList<>();
    public static List<Integer> block1EntryTimes = new ArrayList<>();


    public static void awakeInputBlock(Process process){
        //CPU切换内核态
    ...
    public static void blockProcess(Instruction nowInstruction, Process process){
        ...
        // 记录进程ID和当前时间到阻塞记录表中
        block1ProcessIds.add(process.getProcessID());
        block1EntryTimes.add(Clock_thread.COUNTTIME);
    }

    public static void CheckBlock(int time,Process process) {
        if(Clock_thread.COUNTTIME - time == 2){
            if(!block1Queue.isEmpty()){
                awakeInputBlock(process);
            }
        }
        else {
            return;
    ...
    public static void awakeInputBlock1(Process pcb) {
        //CPU切换内核态
        cpu.changeToKernelState();
        //将当前进程重新插入就绪队列队尾
        //process.RunTime += 3;
    ...
    public static void awakeInputBlock2(Process pcb) {
        //CPU切换内核态
        cpu.changeToKernelState();
        //将当前进程重新插入就绪队列队尾
        //process.RunTime += 3;
    ...
    public static void awakeInputBlock3(Process pcb) {
        //CPU切换内核态
        cpu.changeToKernelState();
        //将当前进程重新插入就绪队列队尾
        //process.RunTime += 3;
